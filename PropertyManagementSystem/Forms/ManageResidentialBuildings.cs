...
{
    public partial class ManageResidentialBuildings : Form
    {
        private readonly MyDbContext _db = new MyDbContext();
        public ManageResidentialBuildings()
        {
            InitializeComponent();
        }


        private void ManageResidentialBuildings_Load(object sender, EventArgs e)
        {
            Populate();
        }
        ...
        private void Populate()
        {
            ...
            var properties = _db.Properties.Where(p => p.BuildingType == BuildingType.Residential).OrderBy(p => p.Id).ToList();
        ...
        {
            new AddResidentialBuilding().ShowDialog();
            Populate();
        }
        ...
        private void EnableAll()
        {
            txtName.Enabled = txtAddress.Enabled = txtBuiltArea.Enabled = txtNotBuiltArea.Enabled =
                cbUnit.Enabled = txtLocationUrl.Enabled =
                    btnEdit.Enabled = btnDelete.Enabled = txtNoOfFloors.Enabled = true;
        }
        ...
        private void DisableAll()
        {
            txtName.Enabled = txtAddress.Enabled = txtBuiltArea.Enabled = txtNotBuiltArea.Enabled =
                cbUnit.Enabled = txtLocationUrl.Enabled =
                    btnEdit.Enabled = btnDelete.Enabled = txtNoOfFloors.Enabled = false;
        }
        ...
        private void txtSearch_OnValueChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(txtSearch.Text))
            {
                var properties = _db.Properties.Where(p =>
                    p.BuildingType == BuildingType.Residential && p.Name.Contains(txtSearch.Text)).OrderBy(p => p.Id).ToList();
                var documents = new List<PropertyDocument>();
                foreach (var property in properties)
                {
                    var doc = _db.PropertyDocuments.FirstOrDefault(pd => pd.PropertyId == property.Id);
