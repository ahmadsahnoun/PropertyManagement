...
{
    public partial class ManageFarms : Form
    {
        ...
        private void Populate()
        {
            ...
            var properties = _db.Properties.Where(p => p.BuildingType == BuildingType.Farm).OrderBy(p => p.Id).ToList();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrWhiteSpace(txtName.Text) && !string.IsNullOrWhiteSpace(txtTotalArea.Text) &&
                !string.IsNullOrWhiteSpace(txtAddress.Text) && !string.IsNullOrWhiteSpace(txtLocationUrl.Text) &&
                cbUnit.selectedIndex > -1)
            {
                ...
                property.TotalArea = double.Parse(txtTotalArea.Text);
        ...
        private void Clear()
        {
            txtName.Text = txtTotalArea.Text =
                txtTotalArea.Text = txtAddress.Text = txtLocationUrl.Text = txtStatus.Text = "";
            dgvOwnershipDocs.Rows.Clear();
            dgvBuildingDrawings.Rows.Clear();
        ...
        {
            var result = MessageBox.Show(@"Are you sure you want to delete this farm?", "",
            ...
            MessageBox.Show(@"Farm Deleted Successfully", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
            Clear();
            Populate();
            DisableAll();
        }
        ...
        private void txtSearch_OnValueChanged(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(txtSearch.Text))
            {
                var properties = _db.Properties.Where(p =>
                        p.BuildingType == BuildingType.Farm && (p.Name.Contains(txtSearch.Text) ||
                                                                p.TotalArea.ToString(CultureInfo.InvariantCulture)
                                                                    .Contains(txtSearch.Text) ||
                                                                p.Location.Contains(txtSearch.Text) ||
                                                                p.LocationUrl.Contains(txtSearch.Text)))
                    .OrderBy(p => p.Id)
                    .ToList();
